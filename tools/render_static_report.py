...
def render_section(section: dict, active: bool) -> str:
    ...
    if section["items"]:
        ...
        for item in section["items"]:
            ...
            title_original = item.get("title_original") or ""
            original_html = ""
            if title_original:
                original_html = f'<span class="news-title-original">Original: {e(title_original)}</span>'
            item_cards.append(
                f'<a class="news-item" href="{e(item["url"])}" target="_blank" rel="noopener">'
                f'<span class="news-title">RU: {e(title_ru)}</span>'
                f'{original_html}'
                f'<span class="news-desc">{e(item.get("description_ru") or "")}</span>'
                f'</a>'
            )
        items_html = "".join(item_cards)
    else:
